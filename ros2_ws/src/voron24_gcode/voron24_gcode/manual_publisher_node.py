...
try:
    from voron24_msgs.msg import PrinterCommand, ExtrusionPoint
    HAS_MSGS = True
...
SPEED_DEFAULT = 1.0
# 시각화 테스트용 출력 폭/높이: 이전 0.01 m의 절반.
DEFAULT_EXTRUSION_WIDTH_M = 0.005
DEFAULT_EXTRUSION_HEIGHT_M = 0.005
...
class ManualPublisher(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('extrusion_width_m', DEFAULT_EXTRUSION_WIDTH_M)
        self.declare_parameter('extrusion_height_m', DEFAULT_EXTRUSION_HEIGHT_M)
        ...
        self.extrusion_width_m = float(self.get_parameter('extrusion_width_m').value)
        self.extrusion_height_m = float(self.get_parameter('extrusion_height_m').value)
        ...
        self._manual_position_m = [0.0, 0.0, 0.0]
        ...
        if HAS_MSGS:
            ...
            self.extrusion_pub = self.create_publisher(ExtrusionPoint, '/printer/extrusion', 200)
        ...
        self.get_logger().info(
            ...
            f'extrusion={self.extruding} width={self.extrusion_width_m}m '
            f'height={self.extrusion_height_m}m')
    ...
    def start_keyboard(self):
        ...
        self.get_logger().info(
            f'keyboard | X a/d  Y s/w  Z q/e  home h  extrusion f  speed [ ] \\  quit Ctrl-C  '
    ...
    def send_jog(self, dx_mm, dy_mm, dz_mm):
        ...
        self._manual_position_m[0] += dx_mm * 0.001
        self._manual_position_m[1] += dy_mm * 0.001
        self._manual_position_m[2] += dz_mm * 0.001
        self.publish_extrusion_point()

    def toggle_extrusion(self):
        """F 키로 압출 상태를 전환하고 현재 노즐 위치에 상태점을 보낸다."""
        self.extruding = not self.extruding
        self.publish_extrusion_point()
        self.get_logger().info(f'extrusion {"ON" if self.extruding else "OFF"}')

    def publish_extrusion_point(self):
        """수동 노즐 위치와 현재 압출 상태를 `/printer/extrusion`에 보낸다."""
        ...
        msg.position = Point(
            x=self._manual_position_m[0],
            y=self._manual_position_m[1],
            z=self._manual_position_m[2],
        )
        msg.width = self.extrusion_width_m
        msg.height = self.extrusion_height_m
        msg.extruding = self.extruding
        msg.layer = 0
