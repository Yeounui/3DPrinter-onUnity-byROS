...
두 곳에 생김. 이 노드는 델타만 mm 로 던지는 입력 장치이며 Unity
KeyboardJointController 와 동등한 peer.
...
# 키맵 — 04a §5b′ 표가 단일 출처. Unity KeyboardJointController.cs 와 의미 동기화.
# 방향키 미사용 이유: 3바이트 ESC 시퀀스로 상태 기계 필요.
AXIS_KEYS = {
...
QUIT_KEY = '\x03'                                     # Ctrl-C. cbreak 는 ISIG 를 살려 두므로
                                                      # 보통은 여기까지 안 오지만 대비해 둠

# 화살표 선택 지원 (04a §5b′). Unity 키맵 의미를 문자 키로 매핑.
ESC_SEQUENCES = {
    '\x1b[D': 'a', '\x1b[C': 'd',       # ← / →
    '\x1b[B': 's', '\x1b[A': 'w',       # ↓ / ↑
...
def parse_keys(buf):
    ...
    while i < len(buf):
        ...
        i += 1                      # 미인식 시퀀스 — ESC 1바이트만 폐기, 나머지 유지
    return keys, ''
...
class ManualPublisher(Node):
    ...
    def start_keyboard(self):
        ...
        tty.setcbreak(self._fd)     # ISIG 는 살아 있어 Ctrl-C 가 그대로 먹음
    ...
    def on_sigterm(self, signum, frame):
        """SIGTERM 은 터미널 시그널이 아니라 cbreak ISIG(Ctrl-C) 방어 범위 밖.
        `kill -TERM` 한 방에 에코 꺼진 터미널 방치를 막는 유일한 경로.

        여기서 복원만 수행. 종료 절차는 원래 핸들러 + main finally 몫.
        """
