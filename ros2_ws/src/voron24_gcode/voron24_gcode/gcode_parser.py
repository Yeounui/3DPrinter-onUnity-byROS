...
이송속도만 여기서 환산 — `F`(mm/min)를 60 으로 나눠 mm/s 로.
파일은 한 줄씩 읽어 Move 제너레이터로 흘림. 전체를 메모리에 올리지 않음.

축별 최대속도 클램프는 motion.py 담당. 좌표 리밋 검사는 상태 노드(A) 담당.
"""
import math
import re
from dataclasses import dataclass

# 워드 하나 = 문자 + 숫자. `G28 X Y` 처럼 값 없는 축 지정이 있어 숫자는 optional.
# 공백 없이 붙여 쓴 `G1X120Y100` 도 인식.
WORD_RE = re.compile(r'([A-Za-z])\s*([-+]?[0-9]*\.?[0-9]*)')

# 궤적에 영향이 없어 조용히 버리는 코드. 목록에 없는 코드는 경고 후 무시 —
# 모르는 코드를 소리 없이 삼키면 방언이 바뀐 것을 알아챌 방법이 없음.
...
# F 가 한 번도 안 나온 상태에서 이동이 나온 경우용. 0 으로 두면 보간기가 그 자리에
# 영원히 멈추므로 값을 하나 넣고 경고. 정상적인 슬라이서 출력에서는 쓰일 일 없음.
FALLBACK_FEED_MM_S = 50.0


class GcodeError(ValueError):
    """그대로 진행하면 조용히 틀린 궤적이 나오는 경우에만 발생.

    무시해도 형상이 유지되는 쪽(G2, 모르는 M 코드)은 경고로 끝내고, 좌표계 자체가
    어긋나는 쪽(G91, G20)만 예외로 끊음.
    """
...
class Move:
    """직선 세그먼트 하나. 좌표는 mm, 속도는 mm/s.

    `extrude_mm` 은 이 구간에서 밀어 넣은 필라멘트 길이. 압출 여부는 `G0`/`G1` 이
    아니라 이 값으로 판정 — E 없는 `G1`(스커트 이동)과 E 붙은 `G0`(방언)이 둘 다
    실제로 존재하기 때문.
    """

    start: tuple            # (x, y, z) [mm]
    end: tuple              # (x, y, z) [mm]
    length_mm: float        # 미리 계산해 둔 값. 보간기가 틱마다 읽음
    feed_mm_s: float        # G-code 의 F(mm/min) 를 /60 한 값
    extrude_mm: float       # 이 구간의 ΔE [mm]. 리트랙션이면 음수
    width_mm: float         # 직전 `;WIDTH:` 태그. 없으면 0
    height_mm: float        # 레이어 높이. `;HEIGHT:` 또는 `;Z:` 차이로 산출
    layer: int              # `;LAYER_CHANGE` 카운트. 첫 층이 1
    line_no: int            # 원본 줄 번호. 경고·디버깅용

    @property
    def extruding(self):
        """ExtrusionPoint.extruding 그대로. false 면 Unity 가 선을 끊음."""
    ...
    def point_at(self, s_mm):
        """시작점에서 s_mm 만큼 진행한 좌표. motion.py 가 쓰는 보간 진입점.

        시작점과 끝점의 X/Y/Z 를 선형 보간.
        """
...
class GcodeParser:
    """모달 상태를 유지하며 한 줄씩 Move 로 변환.

    G-code 는 한 번 지정한 값이 갱신될 때까지 유지되는 모달 방식. 생략된 축은 직전
    위치, 생략된 `F` 는 직전 이송속도를 뜻하므로 파서도 프린터와 같은 상태를 들고
    있어야 함.

    경고는 `warnings` 에 쌓고, `on_warning` 을 주면 발생 즉시 호출 — 노드에서
    `get_logger().warn` 을 물리는 지점.
    """
    ...
    def reset(self):
        ...
        self.origin = [0.0, 0.0, 0.0]       # G92 가 옮긴 원점. 기계 = 지령 + origin
        self.feed_mm_s = 0.0
        self.absolute_e = True              # M82 기본. M83 이면 E 가 증분값
        self.e_command = 0.0                # 마지막 E 지령값. 절대 모드의 기준점
        self.width_mm = 0.0
        self.height_mm = 0.0
        self.layer = 0
        self.layer_z = 0.0                  # 첫 층 높이 = 첫 `;Z:` 값 그 자체
        self.line_no = 0
        self.move_count = 0
    ...
    def parse(self, lines):
        """줄 이터러블 -> Move 제너레이터. 파일 객체를 그대로 넘겨도 됨."""
        for raw in lines:
            self.line_no += 1
    ...
    def feed_line(self, raw):
        ...
        if not words or not words[0][1]:
            self.warn(f'해석 불가한 줄: {code_text!r}')
            return None
        ...
        if code == 'G91':
            raise GcodeError(self.line_no,
                             'G91(상대좌표) 미지원. 슬라이서 프로파일을 G90 으로 둘 것')
        if code == 'G20':
            raise GcodeError(self.line_no,
                             'G20(inch) 미지원. 좌표가 25.4 배로 어긋남')
        if code in ('G2', 'G3'):
            self.warn(f'{code}(원호) 미지원 — 무시함. 프로파일에서 arc fitting 을 끌 것')
            return None
        if code not in IGNORED_CODES:
            self.warn_once(code, f'모르는 코드 {code} — 무시함')
        return None
    ...
    def do_move(self, words):
        ...
        if length == 0.0:
            # 리트랙션(E 만 변함)이나 F 만 지정한 줄. E 는 위에서 이미 반영.
            self.pos = target
            return None
        if self.feed_mm_s <= 0.0:
            self.warn_once('F', f'F 지정 전에 이동 발생 — {FALLBACK_FEED_MM_S}mm/s 로 진행')
    ...
    def do_home(self, words):
        """G28. 세그먼트를 만들지 않고 좌표만 0 으로 스냅.

        홈잉 속도가 G-code 에 없어 세그먼트로 만들 근거가 없음. 파일 첫머리의 G28 은
        이미 0 이라 무해하지만, 이동 후에 나오면 궤적이 튀므로 경고.
        """
        ...
        if any(self.pos[i] != 0.0 for i in axes):
            self.warn('G28 이 0 이 아닌 위치를 버림 — 궤적이 그 지점에서 튐')
        for i in axes:
            self.pos[i] = 0.0
            self.origin[i] = 0.0
        return None
    ...
    def do_set_origin(self, words):
        """G92. 지금 있는 자리에 다른 좌표를 붙이는 지령 (기계는 움직이지 않음).

        대부분은 슬라이서가 층마다 넣는 `G92 E0`. 다만 `G92 X0 Y0` 로 좌표계를 통째로
    ...
    def read_tag(self, comment):
        """슬라이서가 심는 주석 태그. 궤적에 없는 정보라 버리면 복구 불가."""
        text = comment.strip()
        upper = text.upper()
    ...
    def read_tag_value(self, text, prefix_len):
        try:
            ...
            self.warn(f'태그 값을 읽을 수 없음: {text!r}')
            return None
    ...
    def warn_once(self, key, message):
        """같은 코드가 수천 줄 반복되는 일이 흔해 첫 1 회만 출력."""
...
def parse_file(path, on_warning=None):
    ...
    노드가 자기 스레드에서 파일을 읽으므로 바이트 오프셋도 그 루프에서 세는 편이
    정확함. 그래서 파서는 오프셋을 모르고 줄 이터러블만 받음.
    """
...
def dump(path, limit=20):
    """세그먼트 덤프 + 합계. 합계는 make_test_gcode.py 가 찍는 정답지와 대조."""
    import sys
    ...
    try:
        ...
        # 거부 경로도 정상 출력으로 처리. traceback 은 노출하지 않음.
...
# ----------------------------------------------------------------------
# 자체 검증 픽스처. tools/make_test_gcode.py 의 torture 와 같은 방언을 겨냥.
# make_test_gcode.py 는 생성기, 이 모듈은 파서라 서로를 참조하지 않고 각자 들고 있음.
_FIXTURE = """\
G21
G90
M82
...
G92 X0 Y0                 ; 좌표계 리셋. 이후 X0 Y0 는 (100,120)
G1 X20 E2.792             ; 실제로는 X120
M83
G1 X0 Y20 E0.698          ; 이후 E 는 증분
;WIDTH:0.6
G1 X-20 E0.698
...
def _self_test():
    ...
    try:
        ...
        check('G91 거부', False, '에러가 발생하지 않음')
    except GcodeError as exc:
