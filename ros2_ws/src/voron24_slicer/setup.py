...
setup(
    ...
    data_files=[
        ...
        # 프로파일 단일 출처는 레포의 tools/slicer/ (04b §294). 여기서는 설치
        # 트리에서도 닿게 share 로 복사만 함 — profiles.py 탐색 순서상 레포
        # 원본이 있으면 그쪽이 이기므로 값 분기 없음.
    ...
    entry_points={
        'console_scripts': [
            # sim.launch.py executable_path() 가 lib/<pkg>/<exe> 로 훑는 이름들.
