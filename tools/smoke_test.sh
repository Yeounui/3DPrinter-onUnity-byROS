...
echo
echo "[3] 조인트 이름"
NAMES=$(timeout 5 ros2 topic echo /joint_states --once 2>/dev/null | grep -A5 '^name:' | tr -d ' -')
...
TF=$(timeout 6 ros2 run tf2_ros tf2_echo bed_origin nozzle 2>&1 | grep -A1 'Translation' | tail -1)
if [ -n "${TF:-}" ]; then
  pass "$TF"
else
