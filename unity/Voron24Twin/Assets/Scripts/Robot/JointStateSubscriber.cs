...
namespace Voron24.Robot
{
    ...
    public class JointStateSubscriber : MonoBehaviour
    {
        ...
        [Tooltip("/joint_states 수신 로그 주기. 1이면 모든 메시지를 기록합니다.")]
        [SerializeField]
        private int debugLogEveryMessages = 1;

        [Tooltip("Unity 조인트 적용 상태 로그 주기(고정 프레임 기준). 0이면 끕니다.")]
        [SerializeField]
        private int debugLogEveryFixedFrames = 25;
        ...
        private void OnJointState(RosJointState message)
        {
            ...
            if (verbose && debugLogEveryMessages > 0 &&
                messageCount % debugLogEveryMessages == 0)
            {
                var targets = new List<string>();
                foreach (Joint joint in joints)
                {
                    targets.Add($"{joint.name}={joint.target:F6}");
                }

                Debug.Log(
                    $"[JointStateDebug] RX #{messageCount} " +
                    $"raw=[{string.Join(", ", message.position)}] " +
        ...
        private void FixedUpdate()
        {
            ...
            fixedFrameCount++;
            if (verbose && debugLogEveryFixedFrames > 0 &&
                fixedFrameCount % debugLogEveryFixedFrames == 0)
            {
                var states = new List<string>();
                foreach (Joint joint in joints)
                {
                    if (!joint.bound) continue;

                    float actual = joint.current;
                    if (!kinematicMode && joint.body != null &&
                        joint.body.jointPosition.dofCount > 0)
                    {
                        actual = joint.body.jointPosition[0];
                    }

                    states.Add(
                        $"{joint.name}:target={joint.target:F6}," +
                        $"current={joint.current:F6},actual={actual:F6}");
                }

                Debug.Log(
                    $"[JointStateDebug] APPLY frame={fixedFrameCount} " +
                    $"kinematic={kinematicMode} " +
