...
{
    /// <summary>
    /// bed_origin 기준 노즐 위치 계산.
    /// 압출 궤적 렌더링 + UI 좌표 표시에서 사용.
    ///
    /// URDF nozzle/bed_origin은 형상 없는 프레임 전용 링크 (계약 §3).
    /// URDF-Importer가 이들도 GameObject로 생성하므로 Transform 접근 가능.
    /// </summary>
    public class NozzleTracker : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            if (nozzle == null)
                Debug.LogError("[NozzleTracker] 'nozzle' 링크 없음");
            if (bedOrigin == null)
                Debug.LogError("[NozzleTracker] 'bed_origin' 링크 없음");
        }
